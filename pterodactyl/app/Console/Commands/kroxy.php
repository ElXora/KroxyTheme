...
class Kroxy extends Command
{
    protected $signature = "kroxy {action?}";
    protected $description = "All commands for Kroxy Theme for Pterodactyl.";
    ...
    public function handle()
    {
        ...
        if ($action === null) {
            $this->line("\r\n            <title>\r\n            ░█████╗░██████╗░██╗██╗░░██╗\r\n            ██╔══██╗██╔══██╗██║╚██╗██╔╝\r\n            ███████║██████╔╝██║░╚███╔╝░\r\n            ██╔══██║██╔══██╗██║░██╔██╗░\r\n            ██║░░██║██║░░██║██║██╔╝╚██╗\r\n            ╚═╝░░╚═╝╚═╝░░╚═╝╚═╝╚═╝░░╚═╝\r\n\r\n           Thank you for purchasing Kroxy</title>\r\n\r\n           > php artisan kroxy (this window)\r\n           > php artisan kroxy install\r\n           > php artisan kroxy update\r\n           > php artisan kroxy uninstall\r\n            ");
        } else {
            $this->info("\n    Installer\n");
            if ($action === "install") {
                $this->install();
    ...
    public function installOrUpdate($isUpdate = false)
    {
        ...
        $versions = File::directories("./kroxy");
        if (empty($versions)) {
            $this->info("No versions found in /kroxy directory.");
            return;
        }

        $version = basename($this->choice("Select a version:", $versions));
        $this->info("Installing Kroxy Theme {$version}...");
        ...
        exec("rsync -a {$excludeOption} kroxy/{$version}/ ./");

        $directoryPath = app_path("Http/Controllers/Admin/Kroxy");
        File::makeDirectory($directoryPath, 0755, true, true);

        $filesOne = ["KroxyController", "KroxyAdvancedController", "KroxyAnnouncementController", "KroxyColorsController", "KroxyDashboardController", "KroxySocialController"];
        ...
        $filesTwo = ["KroxyComponentsController", "KroxyLayoutController", "KroxyMailController", "KroxyMetaController", "KroxyStylingController"];
    ...
    private function aa($filename, $version, $directoryPath)
    {
        ...
        $localSource = base_path("kroxy/" . $version . "/app/Http/Controllers/Admin/Kroxy/" . $filename . ".php");
