...
<div class="row">
    ...
    <div class="col-xs-12">
        ...
        ">
            ...
            <div class="box-body">
                @if ($version->isLatestKroxy())
                    You are running Kroxy Theme version <code>{{ config('app.kroxy') }}</code>. Your theme is up-to-date!
                @else
                    Your theme is <strong>not up-to-date!</strong> The latest version is <a href="https://builtbybit.com/resources/kroxy-theme-latest-pterodactyl.36705/updates" target="_blank"><code>{{ $version->getKroxy() }}</code></a> and you are currently running version <code>{{ config('app.kroxy') }}</code>.
                @endif
            </div>
        </div>
    </div>
