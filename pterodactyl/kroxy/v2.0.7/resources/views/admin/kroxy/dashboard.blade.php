@extends('layouts.kroxy', ['navbar' => 'components', 'sideEditor' => true])
...
@section('content')
    <form action="{{ route('admin.kroxy.dashboard') }}" method="POST">
        <div class="header">
            <a href={{ route('admin.kroxy.components') }}>
        ...
        <div id="components">
            <x-kroxy.drag-n-drop
                ...
                name="banner"
                label="Egg Banner"
                src="/kroxy/dashboard/banner.png"
            />
            <x-kroxy.drag-n-drop
                wide
                name="statCards"
                label="Statistics cards"
                src="/kroxy/dashboard/statCards.png"
            />
            <x-kroxy.drag-n-drop
                name="infoAdvanced"
                label="Advanced information card"
                src="/kroxy/dashboard/infoAdvanced.png"
            />
            <x-kroxy.drag-n-drop
                wide
                name="graphs"
                label="Graphs"
                src="/kroxy/dashboard/graphs.png"
            />
            <x-kroxy.drag-n-drop
                name="info"
                label="Simple information card"
                src="/kroxy/dashboard/info.png"
            />
            <x-kroxy.drag-n-drop
                name="SFTP"
                label="SFTP Details"
                src="/kroxy/dashboard/SFTP.png"
            />
            <x-kroxy.drag-n-drop
                long
                name="sideGraphs"
                label="Vertical Graphs"
                src="/kroxy/dashboard/sideGraphs.png"
            />
        </div>

        @php
            $widgets = old('kroxy:dashboardWidgets', $siteConfiguration['kroxy']['dashboardWidgets'] ?? []);
        @endphp
        <div id="widget-inputs">
            @foreach ($widgets as $index => $widget)
                <input type="hidden" name="kroxy:dashboardWidgets[{{ $index }}]" value="{{ $widget }}" placeholder="Widget {{ $index + 1 }}" />
            @endforeach
        </div>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            document.querySelector('form').addEventListener('submit', function(e) {
                ...
                dropzoneItems.forEach(function(item, idx) {
                    ...
                    input.type = 'hidden';
                    input.name = `kroxy:dashboardWidgets[${idx}]`;
                    input.value = name;
