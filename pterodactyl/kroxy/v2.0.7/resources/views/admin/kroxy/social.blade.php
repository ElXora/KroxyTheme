@extends('layouts.kroxy', ['navbar' => 'social', 'sideEditor' => false])
...
@php
    $discordBoxOptions = [
        ['img' => '/kroxy/social/discord.png', 'value' => 'true', 'label' => 'Enable'],
        ['img' => '/kroxy/social/discordDisabled.png', 'value' => 'false', 'label' => 'Disable'],
    ];
@endphp

@section('content')
    <form action="{{ route('admin.kroxy.social') }}" method="POST" class="content-box">
        <div class="header">
            <p>Social settings</p>
        ...
        <x-kroxy.form-wrapper 
            ...
            description="Configure social media links and settings for the Kroxy panel."
        >
            <x-kroxy.switch 
                id="kroxy:socialButtons"
                name="kroxy:socialButtons"
                :value="$socialButtons"
                        ...
                        >
                            <div class="row">
                                <div class="col-md-6 input-field">
                                    ...
                                    <input type="url" name="kroxy:socials[{{ $index }}][link]" value="{{ old('kroxy:socials.' . $index . '.link', $social['link'] ?? '') }}" placeholder="https://example.com" />
                                </div>
                                <div class="col-md-6 input-field">
                                    <label>Icon Name</label>
                                    <div class="input-field">
                                        <select
                                            name="kroxy:socials[{{ $index }}][icon]"
                                            value="{{ old('kroxy:socials.' . $index . '.icon', $social['icon'] ?? '') }}"
                                        >
                                            ...
                                            @foreach($icons as $icon)
                                                <option value="{{ $icon }}" @if(old('kroxy:socials.' . $index . '.icon', $social['icon'] ?? '') == $icon) selected @endif>{{ ucfirst($icon) }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="row" style="margin-top: 10px;">
                                <div class="col-md-6 input-field">
                                    <label>Title</label>
                                    <input type="text" name="kroxy:socials[{{ $index }}][title]" value="{{ old('kroxy:socials.' . $index . '.title', $social['title'] ?? '') }}" placeholder="Platform Name" />
                                </div>
                                <div class="col-md-6 input-field">
                                    <label>Description</label>
                                    <input type="text" name="kroxy:socials[{{ $index }}][description]" value="{{ old('kroxy:socials.' . $index . '.description', $social['description'] ?? '') }}" placeholder="Follow us on..." />
                                </div>
                            </div>
                            @if($index > 0)
                                <button 
        ...
        <x-kroxy.form-wrapper 
            ...
            description="Configure the Discord box settings for the Kroxy panel."
        >
        ...
        function addSocialItem() {
            ...
            newItem.innerHTML = `
                <div class="row">
                    <div class="col-md-6 input-field">
                        <label>Link URL</label>
                        <input type="url" name="kroxy:socials[${newIndex}][link]" value="" placeholder="https://example.com" />
                    </div>
                    <div class="col-md-6 input-field">
                        <label>Icon Name</label>
                        <select
                ...
                <div class="row" style="margin-top: 10px;">
                    <div class="col-md-6 input-field">
                        <label>Title</label>
                        <input type="text" name="kroxy:socials[${newIndex}][title]" value="" placeholder="Platform Name" />
                    </div>
                    <div class="col-md-6 input-field">
                        <label>Description</label>
                        <input type="text" name="kroxy:socials[${newIndex}][description]" value="" placeholder="Follow us on..." />
                    </div>
                </div>
                <button 
                    type="button" 
        ...
        function removeSocialItem(button) {
            ...
            items.forEach((item, index) => {
                ...
                inputs.forEach(input => {
                    ...
                    input.setAttribute('name', `kroxy:socials[${index}][${field}]`);
                });
            });
    ...
    <script>
    document.addEventListener('DOMContentLoaded', function () {
        const switchElement = document.querySelector('#kroxy\\:socialButtons');
