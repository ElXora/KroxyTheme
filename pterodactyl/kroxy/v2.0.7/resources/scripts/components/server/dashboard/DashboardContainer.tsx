...
const DashboardContainer = () => {
    const { t } = useTranslation(['kroxy/server/dashboard', 'kroxy/server/console']);
    const dashboardWidgets = useStoreState((state: ApplicationStore) => state.settings.data!.kroxy.dashboardWidgets);
            ...
            {(isNodeUnderMaintenance || isInstalling || isTransferring) ? (
                <div>
                    <Alert type={'warning'} className={'mb-4'}>
                        {isNodeUnderMaintenance
                            ? t('node-under-maintenance', { ns: 'kroxy/server/console' })
                            : isInstalling
                            ? t('running-installation-process', { ns: 'kroxy/server/console' })
                            : t('being-transferred', { ns: 'kroxy/server/console' })}
                    </Alert>
                    <Spinner.Suspense>
                        <Console />
                    </Spinner.Suspense>
                ...
                <div className={'grid lg:grid-cols-2 grid-cols-1 gap-4'}>
                    {dashboardWidgets.length === 0 ? (
                        <a
                            className={'lg:col-span-2 px-8 py-6 bg-gradient-to-br from-gray-800 to-black border border-gray-600 rounded-box group'}
                            href={'/admin/kroxy/dashboard'}
                        >
