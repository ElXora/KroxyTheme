...
const InfoCardAdvanced = () => {
    const { t } = useTranslation(['kroxy/server/dashboard', 'kroxy/utilities']);
    ...
    return(
        <TitledGreyBox title={t('server-info.title')}>
            <div className={'grid grid-cols-2 pb-4 border-b-2 border-gray-600'}>
                <span className={'text-gray-300'}>{t('status', { ns: 'kroxy/utilities' })}:</span>
                <p>
                    ...
                    >
                        {status === 'offline' 
                            ? t('offline', { ns: 'kroxy/utilities' })
                            : status === 'running'
                            ? t('online', { ns: 'kroxy/utilities' })
                            : status === 'starting'
                            ? t('starting', { ns: 'kroxy/utilities' })
                            : status === 'stopping'
                            ? t('stopping', { ns: 'kroxy/utilities' })
                            : ''
                        }
                    </span>
                </p>
