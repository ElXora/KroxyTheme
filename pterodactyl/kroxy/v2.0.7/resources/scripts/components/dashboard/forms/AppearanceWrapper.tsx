...
const AppearanceWrapper = () => {
    const { i18n, t } = useTranslation('kroxy/account');
    const [selectedLanguage, setSelectedLanguage] = useState(i18n.language);

    const modeToggler = String(useStoreState((state: ApplicationStore) => state.settings.data!.kroxy.modeToggler));
    const langSwitch = String(useStoreState((state: ApplicationStore) => state.settings.data!.kroxy.langSwitch));
    const defaultMode = useStoreState((state: ApplicationStore) => state.settings.data!.kroxy.defaultMode);
    const languages = useStoreState((state: ApplicationStore) => state.settings.data!.kroxy.languageOptions);
