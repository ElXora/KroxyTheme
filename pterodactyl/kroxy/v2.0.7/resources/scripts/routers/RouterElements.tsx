...
const useKroxySettings = () => {
    const { dashboardPage, icon } = useStoreState((state: ApplicationStore) => state.settings.data!.kroxy);
...
const NavItem = ({ route }: { route: any }) => {
    ...
    const { isDashboardDisabled, icon } = useKroxySettings();
...
const NavItemWrapper = ({ route }: { route: any }) => {
    const { isDashboardDisabled } = useKroxySettings();
