...
class SoftwareVersionService
{
    ...
    /**
     * Get the latest version of Kroxy Theme.
     */
    public function getKroxy(): string
    {
        try {
            $response = $this->client->request('GET', 'https://kroxy.eu.cc/internal/api/version');
     ...
     * Determine if the current version of the panel is the latest.
     */
    public function isLatestKroxy(): bool
    {
        if (config('app.kroxy') === 'canary') {
            return true;
        }

        return version_compare(config('app.kroxy'), $this->getKroxy()) >= 0;
    }
    /**
